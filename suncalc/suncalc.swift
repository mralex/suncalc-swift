...
open class SunCalc {
  ...
  open class func getMoonTimes(_ timeAndDate:Date, latitude:Double, longitude:Double) -> MoonTimes {
    let t = Calendar.current.startOfDay(for: timeAndDate)

    let hc:Double = 0.133 * Constants.RAD()

    var h0 = SunCalc.getMoonPosition(t, latitude: latitude, longitude: longitude).altitude - hc,
    h1:Double, h2:Double, moonrise:Double = 0, moonset:Double = 0, a:Double, b:Double, xe:Double, ye:Double = 0, d:Double, roots:Double, x1:Double, x2:Double, dx:Double

    for i in stride(from: 1, to: 24, by: 2) {
      h1 = SunCalc.getMoonPosition(DateUtils.hoursLater(t, hours: i), latitude: latitude, longitude: longitude).altitude - hc
      h2 = SunCalc.getMoonPosition(DateUtils.hoursLater(t, hours: i + 1), latitude: latitude, longitude: longitude).altitude - hc
      ...
      if (d >= 0) {
        ...
        if (roots == 1) {
          if (h0 < 0) {
            moonrise = x1 + Double(i)
          } else {
            moonset = x1 + Double(i)
          }

        } else if (roots == 2) {
          moonrise = Double(i) + (ye < 0 ? x2 : x1)
          moonset = Double(i) + (ye < 0 ? x1 : x2)
        }
    ...
    return MoonTimes(moonrise: DateUtils.hoursLater(t, hours: Int(moonrise)), moonset: DateUtils.hoursLater(t, hours: Int(moonset)), alwaysUp: alwaysUp, alwaysDown: alwaysDown)
  }
